...
def get_fvd_logits(videos, i3d, device, length=16):
    videos = preprocess(videos)
    embeddings = get_logits(i3d, videos, device, length)
    return embeddings
...
def get_logits(i3d, videos, device, length=16):
    #assert videos.shape[0] % 16 == 0
    with torch.no_grad():
        logits = []
        for i in range(0, videos.shape[2], length):
            batch = videos[:, :, i:i + length].to(device)
            if batch.shape[2] == length:
